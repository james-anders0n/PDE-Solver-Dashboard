...
def build_forecast_feature_row(
    ...
    target_reference_date: str | pd.Timestamp,
) -> pd.DataFrame:
    """Build one unlabeled forecast row from information available at ``as_of_date``."""
    observations_frame = _normalize_observations(observation_frame)
    resolved_as_of = pd.Timestamp(as_of_date)
    resolved_target = pd.Timestamp(target_reference_date)
    if resolved_target <= observations_frame["date"].min():
        msg = "Forecast target date must follow the first observation date."
        raise ValueError(msg)
    history = _available_history(
        observations_frame,
        resolved_as_of,
        inclusive=spec.cutoff_inclusive,
    )
    ...
    return pd.DataFrame(
        [
            {
                AS_OF_DATE_COLUMN: resolved_as_of,
                TARGET_REFERENCE_DATE_COLUMN: resolved_target,
                TARGET_VALUE_COLUMN: float("nan"),
                **features,
            }
        ]
    )
...
def _normalize_observations(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if "vintage_date" in frame.columns:
        normalized["vintage_date"] = pd.to_datetime(frame["vintage_date"], errors="raise")
...
def _target_rows(frame: pd.DataFrame, target_series_id: str) -> pd.DataFrame:
    ...
    if target.empty:
        ...
        raise ValueError(msg)
    sort_columns = ["release_date"]
    if "vintage_date" in target.columns:
        sort_columns.append("vintage_date")
    sort_columns.append("date")
    sorted_target = (
        target.sort_values(sort_columns)
...
def _has_prior_target_observation(
    ...
    if "vintage_date" in target_history.columns:
        target_history = target_history.loc[target_history["vintage_date"] <= release_date]
...
def _available_history(
    ...
    if inclusive:
        ...
        available = frame.loc[frame["release_date"] < as_of_date].copy()
    if "vintage_date" in available.columns:
        if inclusive:
            available = available.loc[available["vintage_date"] <= as_of_date]
        else:
            available = available.loc[available["vintage_date"] < as_of_date]
    if available.empty:
...
def _latest_vintage_by_reference_date(frame: pd.DataFrame) -> pd.DataFrame:
    sort_columns = ["series_id", "date", "release_date"]
    if "vintage_date" in frame.columns:
        sort_columns.append("vintage_date")
    return (
